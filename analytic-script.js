// --- 1. DATA DUMMY (Metadata Lagu untuk Kalkulasi) ---
const sessionTracks = [
...
// --- 2. FUNGSI LOGIKA ANALYTICS ---
const calculateAnalytics = () => {
    // A. Hitung Rata-rata Sync
    ...
    // C. Logika Status
    let statusText = avgSync >= 90 ? "OPTIMAL" : (avgSync >= 70 ? "NEAR OPTIMAL" : "SUB-OPTIMAL");

    // D. Update ke DOM (HTML)
...
// --- 3. EKSEKUSI SAAT HALAMAN DIBUKA ---
    ...
    if (moodCanvas) {
        new Chart(moodCanvas.getContext('2d'), {
            ...
            options: { 
                ...
                scales: {
                    x: {
                        ticks: { display: true }, // Menampilkan label hari yang benar
                        grid: { display: false }
                    },
                    y: {
                        beginAtZero: true,
